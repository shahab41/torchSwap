...
import { getUserBoosters, useMultipleTapBooster } from '@/libs/action';
...
const MultipleBooster: React.FC<ClickBooster> = ({ user }) => {
    ...
    useEffect(() => {
        const fetchBoosterData = async () => {
            try {
                ...
                if (result) {
                    setDailyBoosters(result.multipleTapBoosters);
                    setNextReset(new Date(result.nextReset));
                }
            } catch (error) {
                console.error("Error fetching multipleTapBoosters data:", error);
            }
        };
    ...
    const handleUseBooster = async () => {
        setLoading(true);
        try {
            const result = await useMultipleTapBooster();
            if (result?.success) {
                toast.success("Tap Faster for Next 20 seconds, for 5x on taps!");
                setDailyBoosters(result.multipleTapBoosters);
                setOpen(!open);
                router.push('/')
            } else {
        ...
        <>
            <div onClick={handleOpen} className='w-[50%] rounded-xl blurry py-4 px-2 flex items-center justify-between border-b flex-col cursor-pointer'>
                <div className="flex items-center h-full">
                    <Image src={'/icon/tap.png'} width={30} height={30} alt="ppx" />
                    <div className="ml-3 flex flex-col">
                        <h1 className="text-sm font-bold text-white">BonkBoost</h1>
            ...
            >
                ...
                >
                    ...
                    <div className="flex justify-center items-center mt-4">
                        <Image src="/icon/tap.png" width={80} height={80} alt="Multitap Image" />
                    </div>

                    <Box display="flex" flexDirection="column" alignItems="center" mt={2}>
                        <h1 className="text-white text-2xl font-bold my-2">BonkBoost</h1>
                        <p className="text-sm text-gray-300 text-center">
                            This booster will boost your taps in 5x, for next 20 seconds and decrease energy {userData?.perClick}. ({userData?.perClick * 5});
                        </p>
                        <div className="flex my-2 w-full items-center justify-center gap-[30px]">
                            <p className="flex text-white text-sm items-center font-semibold">
                                <Image src={'/token.png'} width={15} height={15} alt="token" className="mr-1 w-[20px]" /> {userData?.perClick * 5}
                            </p>
...
export default MultipleBooster